...
async def on_ready():
    ...
    """
    logging.info('Loggedf into Discord as %s', client.user)
...
async def on_message(message):
    ...
    if (message.channel.name not in LISTENING_CHANNELS) or (message.author == client.user):
        return
    await message.reply("I am a faucet bot")
    print(message)
    logging.info('Loggedf into Discord as %s %s %s', message.channel.name,message.author, message)
